...
func TestSetPacksHandler(t *testing.T) {
	tests := []struct {
		name           string
		body           string
		initialPacks   []int
		...
		{"empty sizes should return 400", `{"sizes": []}`, []int{250}, http.StatusBadRequest, "pack sizes cannot be empty", nil},
		{"negative size should return 400", `{"sizes": [100, -5]}`, []int{250}, http.StatusBadRequest, "all pack sizes must be positive integers", nil},
		{"zero size should return 400", `{"sizes": [0]}`, []int{250}, http.StatusBadRequest, "all pack sizes must be positive integers", nil},
		{"valid sizes should return 200", `{"sizes": [100, 200, 300]}`, []int{250}, http.StatusOK, "", []int{100, 200, 300}},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodPut, "/v1/packs", bytes.NewBufferString(tt.body))
			req.Header.Set("Content-Type", "application/json")
			rec := httptest.NewRecorder()

			cfg.SetPacksHandler(rec, req)
